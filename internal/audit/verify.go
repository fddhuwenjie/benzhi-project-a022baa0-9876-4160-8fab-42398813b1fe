...
func (l *Logger) VerifyChain(batchID string, revision int) (int, string, error) {
	return l.VerifyChainContext(context.Background(), batchID, revision)
}

// VerifyChainContext validates the audit chain while allowing callers to stop
// work that has not yet entered a filesystem operation.
func (l *Logger) VerifyChainContext(ctx context.Context, batchID string, revision int) (int, string, error) {
	l.mu.Lock()
	defer l.mu.Unlock()
