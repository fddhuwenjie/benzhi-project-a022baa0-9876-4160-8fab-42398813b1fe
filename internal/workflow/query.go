...
func (s *Service) QueryBatches(o BatchListOptions) (BatchListResult, error) {
	...
	if o.HasUnmet != nil {
		...
		cacheKey = string(raw)
		s.filteredQueryMu.RLock()
		if cacheKey == s.filteredQueryKey && time.Now().Before(s.filteredQueryEnds) {
			cached := cloneBatchListResult(s.filteredQuery)
			s.filteredQueryMu.RUnlock()
			return cached, nil
		}
		s.filteredQueryMu.RUnlock()
	}
	...
	result := BatchListResult{Batches: page, NextCursor: next, Stats: stats, StatusCounts: stats.StatusCounts, IntegrityGradeCounts: stats.IntegrityGradeCounts, UnmetPreconditions: stats.UnmetPreconditions}
	if cacheKey != "" {
		s.filteredQueryMu.Lock()
		s.filteredQueryKey = cacheKey
		s.filteredQuery = cloneBatchListResult(result)
		s.filteredQueryEnds = time.Now().Add(2 * time.Minute)
		s.filteredQueryMu.Unlock()
	}
	return result, nil
}
