...
var cursorProcessKey = func() string {
	var key [16]byte
...
var cursorServiceSequence atomic.Uint64

func newCursorScope() string {
	return fmt.Sprintf("%s-%d", cursorProcessKey, cursorServiceSequence.Add(1))
}

func (s *Service) encodeBatchCursor(b domain.IceCoreBatch) string {
	value := b.CreatedAt.UTC().Format("2006-01-02T15:04:05.999999999Z07:00") + "|" + b.ID + "|" + s.cursorScope
	return base64.RawURLEncoding.EncodeToString([]byte(value))
}
func (s *Service) decodeBatchCursor(v string) (string, string, error) {
	raw, err := base64.RawURLEncoding.DecodeString(v)
	if err != nil {
		return "", "", fmt.Errorf("invalid cursor")
	}
	p := strings.SplitN(string(raw), "|", 3)
	if len(p) != 3 || p[0] == "" || p[1] == "" || p[2] != s.cursorScope {
		return "", "", fmt.Errorf("invalid cursor")
	}
...
func (s *Service) QueryBatches(o BatchListOptions) (BatchListResult, error) {
	...
	if o.Cursor != "" {
		var err error
		ctime, cid, err = s.decodeBatchCursor(o.Cursor)
		if err != nil {
	...
	next := ""
	if len(page) > o.Limit {
		next = s.encodeBatchCursor(page[o.Limit-1])
		page = page[:o.Limit]
	}
