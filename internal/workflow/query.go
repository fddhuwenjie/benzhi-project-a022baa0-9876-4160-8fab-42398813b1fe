...
func (s *Service) ReleaseReport(id string) (audit.SealedReport, error) {
	return s.ReleaseReportContext(context.Background(), id)
}

func (s *Service) ReleaseReportContext(ctx context.Context, id string) (audit.SealedReport, error) {
	if err := ctx.Err(); err != nil {
		return audit.SealedReport{}, err
	}
	b, e := s.GetBatch(id)
	if e != nil {
	...
	n, tail, e := s.audit.VerifyChainContext(ctx, id, b.Revision)
